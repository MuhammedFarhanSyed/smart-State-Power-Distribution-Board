    ...
    <main className="min-h-screen p-6 md:p-10">
      <section className="mx-auto max-w-6xl">
        <header className="mb-8 flex flex-col gap-3 border-b border-slate-200 pb-6 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-blue-700">KSPDB control room</p>
            <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950">Fault simulator</h1>
            <p className="mt-2 max-w-2xl text-slate-600">Select a network asset. Dark nodes preview the expected outage; creating the alert sends telemetry to the Django backend.</p>
          </div>
          <div className="rounded-lg bg-slate-900 px-4 py-3 text-sm text-slate-100">Backend: <span className="font-semibold">127.0.0.1:8000</span></div>
        </header>

        <div className="grid gap-6 lg:grid-cols-[1fr_330px]">
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-lg font-bold">Distribution network</h2>
            <p className="mt-1 text-sm text-slate-500">Feeder = whole column · DT = one transformer · Pole = fault before that pole.</p>
          ...
          <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-sm font-semibold uppercase tracking-wide text-slate-500">Selected simulation</p>
            <p className="mt-3 min-h-16 text-lg font-semibold text-slate-900">{selectionText}</p>
            <button disabled={!selection || isSubmitting} onClick={createAlert} className="mt-5 w-full rounded-lg bg-red-600 px-4 py-3 font-bold text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:bg-slate-300">
              {isSubmitting ? "Creating alert…" : "Create alert"}
            </button>
            <p className="mt-3 text-xs leading-5 text-slate-500">This calls the simulator API. The backend receives telemetry and creates the incident; the UI does not create tickets directly.</p>
            {result && <p className={`mt-5 rounded-lg p-3 text-sm ${result.kind === "success" ? "bg-emerald-50 text-emerald-800" : "bg-red-50 text-red-800"}`}>{result.text}</p>}
          </aside>
        </div>
      </section>
    </main>
