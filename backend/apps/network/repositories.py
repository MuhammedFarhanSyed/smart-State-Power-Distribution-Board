...
from apps.network.models import Substation, Feeder, DistributionTransformer, Pole


class SubstationRepository:
    """
    Repository providing encapsulated database queries for Substations.
    """
...
class FeederRepository:
    """
    Repository providing encapsulated database queries for Feeders.
    """

    @staticmethod
    def get_by_id(feeder_id: str) -> Optional[Feeder]:
        """Fetch a single feeder by ID, selecting related substation."""
        try:
...
class TransformerRepository:
    """
    Repository providing encapsulated database queries for Distribution Transformers.
    """

    @staticmethod
    def get_by_id(dt_id: str) -> Optional[DistributionTransformer]:
        """Fetch a single DT by ID, selecting parent feeder and substation."""
        try:
...
class PoleRepository:
    """
    Repository providing encapsulated database queries for Poles and radial topology lookups.
    """

    @staticmethod
    def get_by_id(pole_id: str) -> Optional[Pole]:
        """Fetch a single pole by ID, selecting parent DT, Feeder, and Parent Pole."""
        try:
    ...
    def get_poles_by_dt(dt_id: str) -> QuerySet[Pole]:
        ...
        Uses select_related to pre-fetch parent poles for tree construction without N+1 queries.
        """
