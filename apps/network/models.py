...
class Substation(TimeStampedModel):
    ...
    def __str__(self) -> str:
        return f"Substation {self.substation_id} - {self.name}"
...
class Span(TimeStampedModel):
    """
    Represents the physical low-tension (LT) wire span between an upstream pole and a downstream pole.
    Identifies the exact physical wire segment where faults occur.
    """
    span_id = models.CharField(
        max_length=128,
        primary_key=True,
        help_text="Unique identifier for the wire span (e.g., SPAN-P-024431-P-024432)."
    )
    from_pole = models.ForeignKey(
        Pole,
        on_delete=models.CASCADE,
        related_name='outgoing_spans',
        help_text="Upstream pole where this wire span originates."
    )
    to_pole = models.ForeignKey(
        Pole,
        on_delete=models.CASCADE,
        related_name='incoming_spans',
        help_text="Downstream pole where this wire span terminates."
    )
    dt = models.ForeignKey(
        DistributionTransformer,
        on_delete=models.CASCADE,
        related_name='spans',
        help_text="Distribution Transformer serving this wire span."
    )
    feeder = models.ForeignKey(
        Feeder,
        on_delete=models.CASCADE,
        related_name='spans',
        help_text="11kV Feeder serving this wire span."
    )
    length_meters = models.DecimalField(
        ...
        null=True,
        blank=True,
        help_text="Physical length of the conductor wire span in meters."
    )

    class Meta:
        db_table = 'network_span'
        verbose_name = 'Span'
        verbose_name_plural = 'Spans'
        ordering = ['dt_id', 'from_pole_id', 'to_pole_id']
        indexes = [
            models.Index(fields=['from_pole', 'to_pole'], name='idx_span_from_to'),
            models.Index(fields=['dt'], name='idx_span_dt'),
            models.Index(fields=['feeder'], name='idx_span_feeder'),
        ]

    def __str__(self) -> str:
        return f"Span {self.span_id} ({self.from_pole_id} -> {self.to_pole_id})"
